...
namespace Framework
{
    ...
    public class Tool
    {
        ...
        */
        ...
        /// 创建子节点列表
        /// </summary>
        public static void CreateChildList(Transform parent, int needChildCount)
        {
            int curChildCount = parent.childCount;
            if (curChildCount < 1)
            {
                Log.Error("创建子节点列表失败，没有子物体摸板");
                return;
            }
            GameObject goTemplet = parent.GetChild(0).gameObject;
            ...
            else
            {
                ...
                {
                    CreateGameObject(goTemplet, parent);
                }
            }
        }
        ...
        /// 创建子物体
        /// </summary>
        /// <param name="goTemplet"></param>
        /// <param name="parent"></param>
        /// <returns></returns>
        public static GameObject CreateGameObject(GameObject goTemplet,Transform parent)
        {
            GameObject go = GameObject.Instantiate(goTemplet);
            go.transform.SetParent(parent);

            go.name = goTemplet.name;
